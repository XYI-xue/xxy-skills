...
* [ ] **Core node connections exist**: Verify `core` has connections to all three columns:
    * [ ] core → Left column (applications/reflections)
    * [ ] core → Center column (architecture/core content)
    * [ ] core → Right column (technical/mechanisms)

* [ ] **Edge IDs are unique**: No duplicate IDs in the edges array

* [ ] **Edge ID semantics match connections**: Each edge's ID should clearly describe its relationship (e.g., `core-pm` means core → pm-perspective)
...
## Optional Enhancement: Knowledge Blindspots

If the source file contains `#?` tags (indicating previous knowledge gaps), see `references/knowledge-blindspots.md` for instructions on how to add a centralized "Knowledge Blindspots" node to highlight learning points.
